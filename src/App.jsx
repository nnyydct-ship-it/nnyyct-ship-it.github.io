...
// 让 public 资源在 GitHub Pages 下也稳（BASE_URL）
const withBase = (p) => {
...
const SubPage = ({ title, icon: Icon, onBack, color }) => (
  <div className="min-h-screen bg-[#FFF0F5] flex flex-col items-center justify-center p-6 relative overflow-hidden animate-fade-in">
    <div
      className={`absolute top-0 left-0 w-full h-64 bg-gradient-to-r ${color} opacity-20 blur-3xl`}
    ></div>
    <button
      onClick={onBack}
    ...
    <div className="bg-white/60 backdrop-blur-xl p-10 rounded-[40px] shadow-xl text-center max-w-lg border border-white">
      <div
        className={`w-24 h-24 mx-auto bg-gradient-to-br ${color} rounded-full flex items-center justify-center text-white shadow-lg mb-6 animate-bounce-slow`}
      >
        <Icon size={40} />
      </div>
...
export default function App() {
  ...
  const renderCard = (item) => (
    ...
    >
      <div
        className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl ${item.color} rounded-bl-[80px] opacity-20 group-hover:opacity-40 transition-opacity`}
      ></div>
      <div className="flex items-center gap-4 relative z-10">
        <div
          className={`w-12 h-12 bg-white rounded-xl flex items-center justify-center shadow-sm group-hover:scale-110 group-hover:rotate-6 transition duration-300 ${item.btnColor}`}
        >
          <item.icon size={24} />
        </div>
        <div className="flex-1">
          <h3 className="text-base font-black text-[#8B4F58] group-hover:text-[#FF5D7D] transition-colors">
            {item.title}
          </h3>
          <p className="text-[10px] text-[#C5A0A6] leading-relaxed">
            {item.desc}
          </p>
        </div>
        <ArrowLeft
          size={16}
          className="rotate-180 text-[#C5A0A6] opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition duration-300"
        />
      </div>
    </div>
  );
  ...
  const renderContent = () => {
    ...
    // ✅ 插画页面
    if (activePage === "art") {
      return <IllustrationsPage onBack={() => setActivePage("home")} />;
    }

    // ✅ Live2D 动画页面（原来叫 gif）
    if (activePage === "gif") {
      return <AnimationsPage onBack={() => setActivePage("home")} />;
    }
      ...
      <div className="min-h-screen bg-[#FFF0F5] font-sans selection:bg-[#FFC2D1] selection:text-[#8B4F58] pb-20 relative">
        ...
        <div className="max-w-6xl mx-auto px-6 py-10 relative z-10">
          ...
          <nav className="relative flex items-center justify-center mb-12 animate-slide-down">
            ...
            <a
              href="https://github.com/nnyydct-ship-it"
              target="_blank"
              rel="noreferrer"
              className="absolute right-0 p-2.5 bg-white/60 hover:bg-white rounded-full text-[#FF8FAB] transition shadow-sm hover:rotate-12"
            >
              <Github size={22} />
            </a>
          </nav>
            ...
            <div className="lg:col-span-4 animate-slide-up">
              <div className="sticky top-8 bg-white/40 backdrop-blur-xl border border-white/60 p-8 rounded-[40px] shadow-[0_20px_40px_rgba(255,182,193,0.15)] text-center hover:-translate-y-1 transition duration-500">
                ...
                >
                  ...
                  {/* ✅ 头像路径改成 public/Avatar/avatar.jpg */}
                  <img
                    src={withBase("Avatar/avatar.jpg")}
                    alt="Avatar"
                    className="w-full h-full rounded-full border-[6px] border-white relative z-20 object-cover bg-[#FFF0F5] shadow-xl transition group-hover/avatar:scale-105"
                  />
                ...
                <h2 className="text-3xl font-black text-[#8B4F58] mb-2">
                  仙界林慕溪
                </h2>
            ...
            <div className="lg:col-span-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="flex flex-col gap-5 animate-slide-up animation-delay-200">
                  <div className="text-xs font-bold text-[#C5A0A6] pl-2 mb-[-10px] uppercase tracking-wider">
                    Literature & Art
                  </div>
          ...
          >
            ...
            >
              <img
                src={withBase("Avatar/avatar.jpg")}
                alt="Big Avatar"
                className="w-full h-auto rounded-[40px] shadow-2xl border-8 border-white/80 select-none"
              />
              <p className="text-white/90 text-center mt-4 font-bold text-lg tracking-widest drop-shadow-md">
                ✨ 仙界林慕溪的美照 ✨
              </p>
            </div>
          </div>
